	...
	if ( isset( $wp_customize->selective_refresh ) ) {
		$wp_customize->selective_refresh->add_partial(
			...
			array(
				...
				'render_callback' => 'hello_world_customize_partial_blogname',
			)
		);
		$wp_customize->selective_refresh->add_partial(
			'blogdescription',
			array(
				...
				'render_callback' => 'hello_world_customize_partial_blogdescription',
			)
		);
	}
}
...
function hello_world_customize_preview_js() {
	wp_enqueue_script( 'hello_world-customizer', get_template_directory_uri() . '/js/customizer.js', array( 'customize-preview' ), HELLO_WORLD_VERSION, true );
}
